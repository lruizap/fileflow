...
#[test]
fn cli_actions_list_shows_echo_copy_move_sync_and_pipeline() {
...
fn cli_run_sync_copies_files() {
    ...
    fs::write(src.join("a.txt"), "hola sync").unwrap();
    ...
    assert_eq!(fs::read_to_string(dst.join("a.txt")).unwrap(), "hola sync");
}