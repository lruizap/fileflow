...
export function useActionFormState() {
  ...
  const merged = {
    ...DEFAULT_FORM_STATE,
    ...state,
    copyOverwrite: state.copyOverwrite ?? state.overwrite ?? false,
    moveOverwrite: state.moveOverwrite ?? state.overwrite ?? false,
    syncOverwrite: state.syncOverwrite ?? state.overwrite ?? false,
  };
  ...
  return {
    ...
    setCopyOverwrite: (value: boolean) => update("copyOverwrite", value),
    setMoveOverwrite: (value: boolean) => update("moveOverwrite", value),
    setSyncOverwrite: (value: boolean) => update("syncOverwrite", value),
